...
* $R_0 < 1$ means that the epidemic will die out
* $R_0 > 1$ means that the epidemic will grow until everyone is infected
...
p(a) &= \frac{f(a)} {\int_{0}^\infty f(a)\mathsf{d}a} = f(a) / R_0.
\end{aligned}
$$

. . .

Can allow $p(t, a)$, hold this fixed for now. 

## The generation interval distribution $p(a)$
...
$$
x(t) = R(t)\int_0^\infty x(t-a)p(a)\mathsf{d}a,
$$
...
$$
x_t = R_t\sum_{a=0}^\infty x_{t-a}\widetilde{p}_a = R_t (x * \widetilde{p}).
$$
...
$$
\mathbb{E}\big[x_t\ |\ x_1,\ldots,x_{t-1}\big] = R_t\sum_{a=0}^\infty x_{t-a}\widetilde{p}_a.
$$
...
$$
\begin{aligned}
\eta_t &= \sum_{a=0}^\infty y_{t-a}\widetilde{p}_a,\\ \\
...
- Smoothness via $\log(\Lambda_t) = \theta^{\mathsf{T}}b_t$
- $b$ is cubic B-spline basis, weighted Ridge penalty on $\theta$
...
tibble(
  `Predicted cases` = c(predict(rto)), 
  lambda = rep(rto$lambda, each = length(rto$observed_counts)),
  Date = rep(rto$x, times = length(rto$lambda))
) |>
  ggplot() +
  #geom_line(data = cancovid, mapping = aes(x = date, y = incident_cases), color = "black") +
...
$$
\textrm{Var}(\widehat{R}) = \left(\textrm{diag}(\widehat{y}) + \lambda D^{\mathsf{T}}D\right)^{\dagger} (\eta^2)
$$
