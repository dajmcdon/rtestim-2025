...
## {background-image="gfx/cover-art-1.svg" footer=false visibility="uncounted" data-hide-slide-number="true"}

\DeclareMathOperator{\argmin}{argmin}
\DeclareMathOperator{\minimize}{minimize}


::: {style="text-align:center; font-family:Gilda Display; padding: 1em 0.5em 0em; background-color: rgba(255, 255, 255, .6); backdrop-filter: blur(5px); box-shadow: 0 0 1rem 0 rgba(0, 0, 0, .5);"}
[{{< meta talk-title >}}]{style="font-size:1.8em;" .primary} 

<br>

[{{< meta author >}}]{style="font-size:1.6em;" .primary}

[Department of Statistics, The University of British Columbia]{style="font-size:1em;" .primary}

![](gfx/qrcodes-1.png){height="350px" fig-align="center"}
:::

## Mathematical modelling of disease / epidemics is very old {style="text-wrap: nowrap;"}
...
![Source: JHU Measles Tracking Team Data](gfx/measles-bubble.svg){width=1000}

:::

## $R_0$ is entirely retrospective {style="text-wrap: nowrap;"}
...
![Source: Guerra, et al., "The basic reproduction number (R0) of measles," (2019).](gfx/gr4_lrg.jpg){height=500}

<!-- https://www.thelancet.com/journals/laninf/article/PIIS1473-3099(17)30307-9/abstract -->
...
<iframe data-src=https://cdcposit.cdc.gov/measles-simulator/ height=850 width=1500></iframe>

## $R(t)$ --- instantaneous reproduction number {style="text-wrap: nowrap;"}
...
$$\begin{aligned}
x(t+h) &\sim  \mathrm{Binom}\left(S(t),\ \frac{\beta}{N} h I(t)\right) = \text{incident infections}\\
z(t+h) &\sim  \mathrm{Binom}\left(I(t),\ \gamma h\right) = \text{incident recoveries}\\
S(t+h) & =  S(t) - x(t+h)\\
I(t+h) & =  I(t) + x(t+h) - z(t+h)\\
R(t+h) & =  R(t) + z(t+h)
\end{aligned}$$
...
$$\begin{aligned}
\frac{\mathsf{d}}{\mathsf{d}t}S(t) & =  -\frac{\beta}{N} S(t)I(t)\\
\frac{\mathsf{d}}{\mathsf{d}t}I(t) & =  \frac{\beta}{N} I(t)S(t) - \gamma I(t)\\
\frac{\mathsf{d}}{\mathsf{d}t}R(t) & =  \gamma I(t)
\end{aligned}$$
:::
...
R_0 &= \beta\ /\ \gamma\\
x_{t+1} 
&= \beta \frac{S_{t}}{N} \sum_{k = 0}^{t-1} \big[(1-\gamma)^{k}\big] x_{t-k} 
= R_0 \frac{S_{t}}{N} \sum_{k = 0}^{t-1} \big[\gamma(1-\gamma)^{k}\big] x_{t-k} 
= R_{t}\sum_{k = 0}^{t-1} g(k)x_{t-k}
\end{aligned}$$

```{r show-sir-Rt}
#| fig-width: 10
sim_SIR <- function(N = 1e3, beta = .3, gamma = .2, i0 = 3) {
  TT <- N
  stopifnot(gamma > 0, gamma < 1)
  states <- matrix(NA, nrow=5, ncol=TT)
  rownames(states) <- c("S", "I", "R", "infections", "expected")
  states[, 1] <- c(N - i0, i0, 0, i0, 1)
  tt <- 1
  while (tt < TT && states["S", tt] > 0 && states["I", tt] > 0) {
    (tt <- tt + 1)
    expected <- states["S", tt-1] * beta * states["I", tt-1] / N
    infections <- rbinom(1, states["S", tt-1], beta * states["I", tt-1] / N)
    removals <- rbinom(1, states["I", tt-1], gamma)
    states["infections", tt] <- infections
    states["expected", tt] <- expected
    states["S", tt] <- states["S", tt-1] - infections
    states["I", tt] <- states["I", tt-1] + infections - removals
    states["R", tt] <- states["R", tt-1] + removals
  }
  as_tibble(t(states)[1:tt, ]) |> 
    mutate(time = row_number(), Rt = (beta * S) / (gamma * N))
}


set.seed(12345)
sir_sim <- sim_SIR()
sir_sim <- pivot_longer(sir_sim, c(S, I, R, Rt, infections)) |>
  mutate(type = case_when(
    name %in% c("S", "I", "R") ~ "compartmental model",
    name == "Rt" ~ "Rt",
    name == "infections" ~ "incident infections"
  ))

p1 <- sir_sim |> 
  filter(type == "compartmental model") |>
  mutate(label = if_else(time == 100, name, NA_character_)) |>
  ggplot(aes(time, value, color = fct_relevel(name, "S", "I", "R"))) +
  geom_line() +
  scale_color_delphi(name = "") +
  scale_y_continuous(expand = expansion(c(0, 0.05)), name = "Prevalence (compartment)") +
  ggrepel::geom_text_repel(aes(label = label), nudge_y = 30) +
  scale_x_continuous(expand = expansion(), name = "") +
  theme(legend.position = "none")

p2 <- ggplot(sir_sim |> filter(type == "incident infections")) +
  geom_segment(aes(time, y = 0, yend = value), color = fourth_colour) +
  scale_y_continuous(
    expand = expansion(c(0, 0.05)), name = "Incident infections"
  ) +
  scale_x_continuous(expand = expansion(), name = "")

p3 <- ggplot(sir_sim |> filter(type == "Rt")) +
  geom_line(aes(time, value), colour = fifth_colour) +
  scale_y_continuous(expand = expansion(c(0, 0.05)), name = "Rt") +
  scale_x_continuous(expand = expansion(), name = "") +
  geom_hline(yintercept = 1)

cowplot::plot_grid(p1, p2, p3, align = "h", axis = "tb", nrow = 1)
```
...
Define $R(t)$ implicitly through the [renewal equation]{.secondary}
...
$$
\mathbb{E}\big[x_{t+1} \mid x_1,\ldots,x_{t}\big] 
= R_t\sum_{a=0}^\infty x_{t-a}\widetilde{g}_a 
= R_t\times (x * \widetilde{g})_t.
$$
...
Raw data: https://www.cdc.gov/cfa-modeling-and-forecasting/rt-estimates/
<iframe data-src="https://www.cdc.gov/cfa-modeling-and-forecasting/rt-estimates/flu_data.json" height=700 width=1400></iframe>
-->
...
#| fig-height: 4
#| fig-align: "center"
cdc_rt_url <- "https://www.cdc.gov/wcms/vizdata/cfa/RtEstimates/flu/flu_timeseries_data.csv"
cdcrt <- read_csv(cdc_rt_url) |>
  filter(state_abb %in% c("CA", "US"))

ggplot(cdcrt, aes(date)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = secondary, alpha = .5) +
  geom_line(aes(y = median), colour = primary) +
  geom_hline(yintercept = 1, colour = tertiary) +
  facet_wrap(~state) +
  scale_x_date(expand = expansion(0), date_breaks = "1 month", date_labels = "%B") +
  ylab("Estimated Rt for influenza\nwith 95% confidence band") +
  xlab("Infection date")
```
...
$x_t$ is [incident infections]{.secondary}, but we don't ever see those
...
::: {.fragment .box-text .absolute top=150 left=150 style="text-wrap: nowrap;"}
...
$$\begin{aligned}
\eta_t &= \sum_{a=0}^{t-1} y_{t-a}p_a,\\ \\
y_t\ |\ y_{t-1},\ldots,y_{1} &\sim \textrm{Poisson}(R_t\eta_t).
\end{aligned}$$
...
::: {.fragment .box-text .absolute top=50}
* This won't give you "$R_t$", because you didn't use incident infections.

* At least, the timing is wrong, because the observable happened _after_ infection.

* Better to deconvolve incidence first ([LSM2026](https://doi.org/10.1101/2025.09.01.25334793)).

[More on this later.]{.claret}
...
::: flex
::: {.w-40 style="text-wrap: nowrap;"}
1. `{EpiEstim}` [CFFC2013](https://doi.org/10.1093/aje/kwt133)
:::
::: w-60
...
::: flex
::: {.w-40 style="text-wrap: nowrap;"}
2. `{EpiFilter}` [Parag 2020](https://doi.org/10.1371/journal.pcbi.1009347.)
:::
::: w-60
- State space model
...
::: flex
::: {.w-40 style="text-wrap: nowrap;"}
3. `{EpiLPS}` [GFH2023](https://doi.org/10.1002/bimj.202200024)
:::
::: w-60
...
::: {.fragment .absolute .box-text top=50 style="text-wrap: nowrap;"}
`{EpiNow2}` [CDC + CFA](https://epiforecasts.io/EpiNow2/), [Abbott, et al., 2023](https://doi.org/10.12688/wellcomeopenres.16006.2)
...
$$\begin{aligned}
\theta 
&\longleftarrow \argmin_\theta\ \eta^{\mathsf{T}}\exp(\theta) - 
\mathbf{y}^{\mathsf{T}}\theta + 
\frac{\rho}{2}\Vert D^{(k)}\theta - \alpha + u \Vert_2^2 \\
\alpha 
&\longleftarrow \argmin_\alpha\ \lambda\Vert D^{(1)} \alpha \Vert_1 +
  \frac{\rho}{2}\Vert D^{(k)}\theta - \alpha + u \Vert_2^2 \\
u 
&\longleftarrow u + D^{(k)}\theta - \alpha
\end{aligned}$$
...
$$\begin{aligned}
\theta &\longleftarrow  
\Big[{\color{Cerulean}\textrm{Proximal Newton / Fisher Scoring}}\Big] \\
\alpha &\longleftarrow  
\Big[{\color{BurntOrange}\textrm{Fused Lasso Signal Approximator}}\Big] \\
...
::: {.fragment style="text-wrap: nowrap;"}
...
delay_distns_can |>
  ...
  ggplot(aes(Delay, Probability, fill = type)) +
...
<img src="gfx/NSERC_RGB.svg" style="height: 150px;"/>
