...
![JHU Measles Tracking Team Data](gfx/measles-bubble.svg){width=1000}
...
g(a) &= \frac{f(a)} {\int_{0}^\infty f(a)\ \mathsf{d}a} = \frac{f(a)}{R_0}.
\end{aligned}$$
...
set.seed(12345)
out <- sim_SIR()
comp <- out |> select(S:R, time) |> pivot_longer(-time) |>
...
rt_inf <- out |> select(infections, Rt, time) |>
  ggplot(aes(time)) +
  geom_line(aes(y = Rt), colour = secondary) +
  geom_hline(yintercept = 1) +
  geom_segment(aes(y = 0, yend = infections / max(infections) * 3 / 2),
   colour = fourth_colour) +
  theme_bw() + ylab("Rt / scaled incident infections") +
  theme(plot.background = element_blank()) +
  scale_y_continuous(expand = expansion(c(0, 0.05))) +
  scale_x_continuous(expand = expansion(0))

cowplot::plot_grid(comp, rt_inf)
```
...
* At least, the timing is wrong, because the observable happend _after_ infection.
